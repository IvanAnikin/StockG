...
from tensorflow.keras.layers import BatchNormalization, Conv2D, MaxPooling2D, Activation, Dropout, Dense, Flatten, Input, LSTM, Dense, Bidirectional
...
class NN():
    ...
    def model_builder(self):
        ...
        inputs = Input(shape=(self.args['window_size'], len(self.args['features'])))

        for (i, f) in enumerate(self.args['filters']):

            if i == 0:
                x = inputs
            elif(i == len(self.args['filters'])-1):
                if(self.args['bidirectional']):
                    x = Bidirectional(LSTM(f, return_sequences=False))(x)
                else:
                    x = LSTM(f, return_sequences=False)(x)
            else:
                if(self.args['bidirectional']):
                    x = Bidirectional(LSTM(f, return_sequences=True))(x)
                else:
        ...
        model.compile(loss=self.args['loss'], metrics=['mean_absolute_error'], optimizer=self.args['optimizer'])
        return model
    ...
    def load_data(self, scale=True, shuffle=True, lookup_step=1, split_by_date=True):
        feature_columns = self.args['features']
        n_steps = self.args['window_size']
